...
class Imaging():
  def __init__(
    ...
    self.if_seg = True if seg_path is not None else False
    self.if_psf = True if psf_path is not None else False
...
class Data_Box(Imaging):
  ...
  def __init__(
    ...
    self.mode = "fitting"
    self.if_fix_when_bootstrap = True
  ...
  # def to_array2d(self, data):
  #   return aa.Array2D.no_mask(
  #       values=data,
  #       pixel_scales=self.arc2pix
  #   )   
  ...
  def __getitem__(self, item):
    ...
    if self.if_psf:
      # psf_data = self.read_data(self.psf_path, self.galaxy_index[item], self.psf_hdu) if self.if_psf else torch.zeros((1)).to(device)
    ...
    if self.mode == "fitting":
      return self.galaxy_index[item], data_matrix, psf_data, self.fit_flex_p(item), self.fit_fix_p(item)
    elif self.mode == "bsp_uc":
      return self.galaxy_index[item], data_matrix, psf_data, self.bsp_flex_p(item), self.bsp_fix_p(item)
    elif self.mode == "cm_uc":
      return self.galaxy_index[item], data_matrix[1:], psf_data, self.cm_flex_p(item), self.fit_fix_p(item)
