...
class Profiles(): 
  def __init__(self, **kwargs):
    ...
    for name, val in kwargs.items():
      ...
      unique_flex = set(self.variableParam.values()) | set(flexx.values())
      unique_flx = set(self.fixedParam.values()) | set(fixx.values())
      
      self.variableParam = {index: value for index, value in enumerate(unique_flex, start=1)}      
      self.fixedParam = {index: value for index, value in enumerate(unique_flx, start=1)}
...
class Data_Box(Imaging):
  ...
  def __init__(
    ...
    super().__init__(galaxy_index=galaxy_index, image_path=image_path, psf_path=psf_path, 
                     ...
                     sigma_hdu=sigma_hdu, seg_hdu=seg_hdu, seg_mode=seg_mode)
  ...
  def fit_fix_p(self, item):
    fix_p = torch.empty((len(self.Profiles.fixedParam))).to(device)
    
    for ams, param in enumerate(self.Profiles.fixedParam.values()):
  ...
  def fit_flex_p(self, item): #fit_flex_p
    flex_p = torch.empty((len(self.Profiles.variableParam))).to(device)
    for ams, param in enumerate(self.Profiles.variableParam.values()): 
  ...
  def __getitem__(self, item):
    ...
    fits_data = self.read_data(self.image_path[item], self.image_hdu)
    # torch.sum(psfim_data) != 0:

    # print(np.shape(fits_data), "shaoe")
    sigma_data = self.sigma_pretreatment(self.read_data(self.sigma_path[item], self.sigma_hdu))

    data_matrix = torch.cat([fits_data.unsqueeze(0), sigma_data.unsqueeze(0)], dim=0)  
    if self.conv_psf:
      psf_data = self.read_data(self.psf_path[item], self.psf_hdu)
    else:
    ...
    if self.use_seg:   
      segim_data = self.read_data(self.seg_path[item], self.seg_hdu)

      if self.seg_index is not None:
        if self.seg_mode == "bg":
            segim_data[segim_data != (int(self.seg_index[item]) | 0)] = 999
            segim_data[segim_data != 999] = 1
            segim_data[segim_data == 999] = 0
        else:
    ...
    if self.mode == "fitting":
      return self.galaxy_index[item], data_matrix, psf_data, self.param_matrix_from_(self.Profiles.variableParam, "proj_Ini_value", item), self.fit_fix_p(item)
