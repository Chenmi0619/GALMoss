...
class EffR(AbstractParameters):
  ...
  def Fitted_Value_cpu(self):
    return self.Fitted_value_gpu.detach().cpu().numpy() * self.arc2pix
...
class PA(AbstractParameters):
  ...
  @property
  def Fitted_Value_cpu(self):
    return self.Fitted_value_gpu.detach().cpu().numpy() * 180 / np.pi 
...
class Mag(AbstractParameters):
  ...
  @property
  def Fitted_Value_cpu(self):
    return self.Fitted_value_gpu.detach().cpu().numpy()  + self.Magnitude_zero
