    ...
    def __init__(
        ...
        if value_range != (None, None):
            self.normalization_relationship = (
                (value_range[1] - value_range[0]) / 2,
                1 - value_range[1] / ((value_range[1] - value_range[0]) / 2)
            )
            condition = (
                torch.max(self._parameters) <= value_range[1] and
                torch.min(self._parameters) >= value_range[0]
            )
