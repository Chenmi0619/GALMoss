...
class Bootstrap:
    ...
    def calculate_uncertainty(self, P_dict):
        ...
        for param in P_dict.values():
            ...
            if param.dim == "1d":
                param.uncertainty = uncertainty.reshape(param.param_length)
    ...
    def refitted_param_append(self, 
        ...
        if not total:
            for param in P_dict.values():
                if hasattr(param, 'bsp_batch_best_value'):
...
class CovarianceMatrix:
    ...
    def store_uncertainty(self):
        ...
        for idx, param in enumerate(self.dataset.Profiles.variableParam
                                    .values()):
            param.uncertainty = self.total_uncertainty[:, idx]
    ...
    def jacobian(self, param: torch.Tensor) -> torch.Tensor:
        ...
        for param_class in self.dataset.Profiles.variableParam.values():
            value = param[ams: ams + param_class.dim2_length]
            if param_class.dim == "1d":
                value = value.reshape(-1, 1)
            param_class.update_updating_val(value)
